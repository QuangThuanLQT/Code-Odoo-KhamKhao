    ...
    def _get_number_next_actual(self):
        ...
        for seq in self:
            if seq.implementation != 'standard':
                ...
                query = "SELECT last_value, 1 as increment_by, is_called FROM ir_sequence_%03d" % seq.id
    ...
    def _get_number_next_actual(self):
        ...
        for seq in self:
            if seq.sequence_id.implementation != 'standard':
                ...
                # get number from postgres sequence. Cannot use currval, because that might give an error when
                # not having used nextval before.
                self._cr.execute("SELECT last_value, 1 as increment_by, is_called FROM ir_sequence_%03d_%03d" % (seq.sequence_id.id, seq.id))
                (last_value, increment_by, is_called) = self._cr.fetchone()
                if is_called:
                    seq.number_next_actual = last_value + increment_by
                else:
